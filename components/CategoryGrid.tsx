      ...
      <CategoryScroller>
        {list.map((c) => (
          <Link key={c.slug} href={`/category/${c.slug}`} title={c.name}
            className="group flex w-[124px] shrink-0 snap-start flex-col rounded-lg border border-slate-100 bg-white p-2.5 shadow-card transition-all duration-200 hover:-translate-y-1 hover:border-green/40 hover:shadow-float sm:w-[132px]">
        ...
        <Link href={viewAllHref} className="flex w-[124px] shrink-0 snap-start flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-green/40 bg-green/5 p-2.5 text-center text-green transition-colors hover:bg-green/10 sm:w-[132px]">
